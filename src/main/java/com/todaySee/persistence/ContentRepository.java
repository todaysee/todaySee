...
public interface ContentRepository extends CrudRepository<Content, Integer>{
	 ...
	 */
	@Query(nativeQuery = true
			,value="SELECT DISTINCT c.* FROM contentgenre cg INNER JOIN content c ON c.content_number = cg.content_number INNER JOIN genre g ON g.genre_number = cg.genre_number WHERE c.content_poster_images_url<>'none Main img' ORDER BY c.content_release_date DESC LIMIT 60,5")
